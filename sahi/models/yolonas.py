...
class YoloNasDetectionModel(DetectionModel):
    def __init__(
        ...
        if model_name is not None and not isinstance(model_name,str):
            raise TypeError(f"model_name should be a string, got {model_name} with type of '{model_name.__class__.__name__}'")
        ...
        if not model_path: # use pretrained models downloaded from Deci-AI remote client
            ...
            if not class_names_yaml_path:
                raise ValueError("'class_names_yaml_path' should be provided for the models that have custom class mapping")
            with open(class_names_yaml_path,"r") as fs:
                yaml_content = safe_load(fs)
                if not isinstance(yaml_content,list):
                    raise ValueError("Invalid yaml file format, make sure your class names are given in list format in yaml")
    ...
    def load_model(self):
        ...
        try:
            model = models.get(model_name=self.model_name,checkpoint_path=self.model_path,pretrained_weights=self.pretrained_weights, num_classes=self.num_classes).to(device=self.device)
