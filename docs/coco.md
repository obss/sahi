...
`sahi coco evaluate` uses pycocotools by default. To use the optional [ultrafast-pycocotools](https://github.com/developer0hye/ultrafast-pycocotools) backend, install `pip install "sahi[ultrafast]"` and pass `--backend ultrafast` (or `backend="ultrafast"` to `sahi.scripts.coco_evaluation.evaluate`).

```bash
sahi coco evaluate --dataset_json_path coco_dataset.json --result_json_path coco_predictions.json --backend ultrafast
```
