...
class TestCocoUtils(unittest.TestCase):
    ...
    def test_merge_from_list(self):
        ...
        self.assertEqual(
            merged_coco_dict["annotations"][12]["id"], 15,
        )
        self.assertEqual(
            merged_coco_dict["annotations"][12]["image_id"], 5,
        )
        self.assertEqual(
            merged_coco_dict["annotations"][9]["category_id"], 1,
        )
        self.assertEqual(
            merged_coco_dict["annotations"][9]["image_id"], 3,
        )
    ...
    def test_get_upsampled_coco(self):
        ...
        self.assertEqual(
            len(upsampled_coco.json["images"]), 250,
        )
        self.assertEqual(
            len(coco.images[5].annotations), len(upsampled_coco.images[5 + len(coco.images)].annotations),
        )
        self.assertEqual(
            coco.image_dir, image_dir,
        )
        self.assertEqual(
            upsampled_coco.image_dir, image_dir,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            upsampled_coco.stats["num_annotations"], len(upsampled_coco.json["annotations"]),
        )
    ...
    def test_get_area_filtered_coco(self):
        ...
        self.assertEqual(
            len(area_filtered_coco.json["images"]), 17,
        )
        self.assertGreater(
        ...
        self.assertEqual(
            len(area_filtered_coco.json["images"]), 24,
        )
        self.assertGreater(
