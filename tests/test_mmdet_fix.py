...
def test_get_sliced_prediction_mmdet_fixed():
    ...
    # Initialize model
    model = MmdetDetectionModel(model_name="yolox_tiny_8x8_300e_coco", confidence_threshold=0.5, device="cpu")
    ...
    assert len(result.object_prediction_list) >= 7, (
        f"Expected at least 7 objects, got {len(result.object_prediction_list)}"
    )
